...
@Component({
  selector: 'app-form-control-holder',
...
export class FormControlHolderComponent implements AfterContentInit {
  ...
  @ContentChildren(FormControlErrorComponent) errorDefinitions: QueryList<FormControlErrorComponent>;
  @Input() label: string;
  name: string;
  required: boolean;
  ...
  setLengthError(errorName: string, value: (errorValue: { requiredLength: number, actualLength: number} | null) => void){
    const error = this.control.getError(errorName);
    if(error){
      value({ requiredLength: error.requiredLength, actualLength: error.actualLength});
    }else{
      value(null);
    }
  }
